...
import { X, BookOpen, TrendingUp, Calendar, Clock, Sparkles, Loader2, BarChart3, Share2 } from 'lucide-react';
...
export default function GardenAlbum({ pomoSessions, pomoHistory, subjects, onClose }) {
    ...
    const handleShare = async () => {
        ...
        setIsCapturing(true);
        try {
            // html2canvas-pro를 사용하여 캡처 (oklch 파싱 호환)
            const canvas = await html2canvas(albumRef.current, {
                scale: 2,
                backgroundColor: document.documentElement.classList.contains('dark') ? '#0f172a' : '#ffffff',
                useCORS: true,
                windowWidth: albumRef.current.scrollWidth,
                windowHeight: albumRef.current.scrollHeight,
            });

            const dataUrl = canvas.toDataURL('image/png');

            // Web Share API가 사용 가능한 환경(모바일 브라우저 등)인지 확인
            if (navigator.share) {
                try {
                    const blob = await (await fetch(dataUrl)).blob();
                    const file = new File([blob], 'pomodoro-cottage-stats.png', { type: 'image/png' });
                    await navigator.share({
                        title: '나의 뽀모도로 정원 🌿',
                        text: '함께 성장하는 나의 뽀모도로 코티지 정원을 구경해보세요!',
                        files: [file]
                    });
                } catch (shareError) {
                    console.log('공유가 취소되었거나 지원하지 않습니다. 다운로드로 대체합니다.', shareError);
                    downloadImage(dataUrl);
                }
            } else {
                // 지원하지 않는 경우 PNG 파일 다운로드
                downloadImage(dataUrl);
            }
        } catch (error) {
    ...
    const downloadImage = (dataUrl) => {
        ...
        link.download = `pomodoro-cottage-album-${new Date().getTime()}.png`;
        link.href = dataUrl;
        link.click();
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[80] flex items-center justify-center p-4 animate-in fade-in duration-300"
            onClick={onClose}>
            <div className={`w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-4 duration-500 custom-scrollbar ${isCapturing ? 'max-h-none overflow-visible' : 'max-h-[90vh]'}`}
                onClick={e => e.stopPropagation()}>

                {/* 📸 캡처 대상 영역 */}
                <div ref={albumRef} className="bg-white dark:bg-slate-900 text-slate-800 dark:text-white pb-6 rounded-[2rem] overflow-hidden">

                    {/* 헤더 */}
                    <div className="sticky top-0 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 px-6 py-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2" data-html2canvas-ignore="true">
                            <button onClick={handleShare} disabled={isCapturing} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-bold bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors disabled:opacity-50 cursor-pointer">
                                {isCapturing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Share2 className="w-4 h-4" />}
                                공유카드
                            </button>
                            <button onClick={onClose} disabled={isCapturing} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer disabled:opacity-50">
